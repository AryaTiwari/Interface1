...
import { StreamingResponseOverlay } from './components/StreamingResponseOverlay';
...
const MainInterface: React.FC = () => {
  ...
  return <div id="ultron-root-app" className={`relative w-screen h-screen overflow-hidden bg-[#030712] text-slate-100 flex flex-col justify-between font-mono select-none transition-colors duration-300 ${alertClass} ${isVibrating ? 'ultron-haptic-vibration' : ''}`} style={{backgroundImage:`radial-gradient(circle at 50% 45%, ${colors.ambient} 0%, transparent 65%), linear-gradient(to bottom, #030712 0%, #080f1e 50%, #030712 100%)`}}>
    <PerformanceGovernor />
    {isAlertActive && <div className={`pointer-events-none fixed inset-0 z-50 ${borderPulseClass}`} />}
    ...
    <div className="absolute inset-0 pointer-events-none opacity-15 gpu-layer" style={{backgroundImage:`linear-gradient(to right, ${colors.border} 1px, transparent 1px), linear-gradient(to bottom, ${colors.border} 1px, transparent 1px)`,backgroundSize:'40px 40px'}} />
    <RuntimeBridge />
    <CredentialManager />
    <TopHeader />
    <main id="center-stage" className="relative flex-1 flex flex-col items-center justify-center w-full px-4 overflow-hidden">
      ...
      <div id="ultron-core-container" className="relative z-20 my-auto flex flex-col items-center gpu-layer"><UltronCore /></div>
    </main>
