...
export const AgentActivity: React.FC = () => {
  ...
  useEffect(() => {
    const handler = (event: Event) => {
      ...
      if (normalized.state === 'complete' || normalized.state === 'error') {
        setActive(null);
  ...
  return <div className="fixed right-5 top-20 z-[70] w-[min(360px,calc(100vw-2rem))] pointer-events-none font-mono">
    <div className="rounded-2xl border border-cyan-500/20 bg-slate-950/85 backdrop-blur-xl shadow-2xl overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2 border-b border-slate-900">
        <div className="flex items-center gap-2">
          <span className={`w-2 h-2 rounded-full ${live ? 'bg-cyan-400 animate-pulse' : 'bg-slate-600'}`} />
          <span className="text-[9px] font-black tracking-[0.2em] text-slate-300">LIVE ACTIVITY</span>
        </div>
      ...
      <div className="px-4 py-3">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[11px] font-bold text-white tracking-wide">{active?.label || active?.text || (state === 'complete' ? 'Task complete.' : 'Standing by.')}</div>
            ...
            <div className="mt-2 flex items-center gap-3 text-[8px] tracking-[0.14em] text-slate-500">
              <span>{formatDuration(elapsed)}</span>
              {active?.toolCalls?.length ? <span>{active.toolCalls.length} TOOL{active.toolCalls.length === 1 ? '' : 'S'}</span> : null}
              {active?.partial ? <span>PARTIAL</span> : null}
            </div>
          </div>
        </div>
        {recent.length > 1 && <div className="mt-3 pt-3 border-t border-slate-900 space-y-1.5">
          {recent.slice(0, -1).reverse().map((item, index) => {
            const itemState = stateOf(item);
            return <div key={`${item.at}-${index}`} className="flex items-center gap-2 text-[8px] tracking-wider text-slate-500">
              <span>{ICONS[itemState] || '•'}</span><span>{LABELS[itemState] || itemState}</span>
              {item.tool && <span className="truncate text-slate-600">{item.tool}</span>}
            </div>;
          })}
        </div>}
      </div>
